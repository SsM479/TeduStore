...
public class UserController extends BaseController {
	...
	public ResponseResult<Void> checkEmail(String email) {
		...
		// 判断验证结果
		if (result) {
			rr = new ResponseResult<Void>(0, "邮箱已经被注册");
		} else {
			rr = new ResponseResult<Void>(1, "邮箱可以正常使用");
		}
		// 返回
		return rr;
	}
	...
	public ResponseResult<Void> checkCode(String code,HttpSession session){
		...
		if(c == null) {
			rr = new ResponseResult<Void>(0, "验证失败!");
		}
		// 忽略大小写比较字符串
		if(c.equalsIgnoreCase(code)) {
			rr = new ResponseResult<Void>(1);
		}else {
			rr = new ResponseResult<Void>(-1, "验证失败");
		}
		return rr;
	}
	...
	// 处理图片下载
	@RequestMapping(value="/demo.do",produces="image/png")// produces 用户设定content-type属性
	@ResponseBody
	public byte[] pngDemo() throws Exception{
		// 读取一个png图片，返回图片数据即可
		String path = "cn/tedu/store/controller/a.png";
		// 从包中读取文件
	...
	private String createCode(int n) {
		String chs = "abcdefghijkmnpqABCDEFGHJKLMNPQ34568";
	 ...
	 *   用于解析文件上载请求
	 * 2.配置SpringMVC上载解析器，使用fileupload组件
	 *   - 设置最大上载限制
	 *   - 设置文件名中文编码
	 * 3.控制器中使用MultiparFile接收上载的文件数据.
	 *   - 注意：变量名与客户端name属性一直
	 *   - 文件的全部信息可以通过Multiparfile对象获得
	 */
	
	@RequestMapping("/upload.do")
	@ResponseBody
	public ResponseResult<Void> upload(MultipartFile image,String memo, HttpServletRequest request) throws IOException{
		// MultipartFile 封装了全部的尚在文件信息
		// 利用其方法可以获取全部的上载信息
		// 显示上载文件的文件名
	...
	@ResponseBody
	public ResponseResult<Void> uploadImages(@RequestParam(value="images") MultipartFile[] images,String memo,HttpServletRequest request) throws IOException{
